document.addEventListener('DOMContentLoaded', () => {
  ...
  const tostudi-kasusBtn = document.getElementById('tostudi-kasusBtn');
  ...
  // Render studi-kasus cards
  const studi-kasusGrid = document.querySelector('#studi-kasus .cards-grid');
  function renderstudi-kasus(filter = 'all') {
    if (!studi-kasusGrid) return;
    studi-kasusGrid.innerHTML = '';
    const filteredItems = filter === 'all' ? studi-kasusItems : studi-kasusItems.filter(item => item.tags.includes(filter));

    if (filteredItems.length === 0) {
      studi-kasusGrid.innerHTML = '<p class="text-center w-100">Tidak ada proyek yang cocok dengan filter ini.</p>';
      return;
    }
    ...
    filteredItems.forEach((item) => {
      ...
      studi-kasusGrid.appendChild(card);
    });
    AOS.refresh(); // Refresh AOS setelah item ditambahkan
  }
  if (studi-kasusGrid) renderstudi-kasus();


  // Handle studi-kasus filter buttons
  document.querySelectorAll('#studi-kasus .filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#studi-kasus .filter-btn').forEach(b => {
        b.classList.remove('active');
        b.setAttribute('aria-pressed', 'false');
      });
      btn.classList.add('active');
      btn.setAttribute('aria-pressed', 'true');
      renderstudi-kasus(btn.dataset.filter);
    });
  });

  // studi-kasus modal elements
  const studi-kasusModalElement = document.getElementById('studi-kasusModal');
  const studi-kasusModal = studi-kasusModalElement ? new bootstrap.Modal(studi-kasusModalElement) : null;
  const modalTitle = document.getElementById('studi-kasusModalLabel');
  const modalImg = document.getElementById('studi-kasusModalImg');
  const modalDesc = document.getElementById('studi-kasusModalDesc');
  const modalTags = document.getElementById('studi-kasusModalTags');

  // Open modal on card click/enter
  function openstudi-kasusModal(cardElement) {
    if (!cardElement || !studi-kasusModal) return;
    const itemId = cardElement.dataset.id;
    const item = studi-kasusItems.find(p => p.id === itemId);
    if (!item) return;
    ...
    studi-kasusModal.show();
  }

  if (studi-kasusGrid) {
    studi-kasusGrid.addEventListener('click', e => {
      const card = e.target.closest('.card');
      if (card) openstudi-kasusModal(card);
    });

    studi-kasusGrid.addEventListener('keydown', e => {
      if ((e.key === 'Enter' || e.key === ' ') && e.target.classList.contains('card')) {
        e.preventDefault();
        openstudi-kasusModal(e.target);
      }
    });
  }

  // Return focus when modal is hidden
  if (studi-kasusModalElement) {
    studi-kasusModalElement.addEventListener('hidden.bs.modal', () => {
  ...
  if (tostudi-kasusBtn) {
    tostudi-kasusBtn.addEventListener('click', () => {
